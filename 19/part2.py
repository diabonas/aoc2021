...
while len(unexplored) > 0:
    for i in unexplored.copy():
        ...
        if scanner_position is not None:
            ...
            scanner_positions[i] = scanner_position

max_dist = 0
for positions in itertools.combinations(scanner_positions, 2):
    dist = sum(abs(positions[1][i] - positions[0][i]) for i in range(3))
    max_dist = max(dist, max_dist)

print(f"largest manhattan distance between two scanners: {max_dist}")
