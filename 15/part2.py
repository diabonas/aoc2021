...
with open("input", "r") as file:
    risk_levels_tile = [[int(l) for l in line.strip()] for line in file.readlines()]

size = len(risk_levels_tile)
assert all(len(row) == size for row in risk_levels_tile)
risk_levels = [[None] * size * 5 for row in range(size * 5)]
for row in range(len(risk_levels)):
    for column in range(len(risk_levels)):
        risk_levels[row][column] = (
            risk_levels_tile[row % size][column % size] + row // size + column // size
        )
        while risk_levels[row][column] > 9:
            risk_levels[row][column] -= 9
